...
public class MoviesPopularAdapter extends RootSelectionAdapter {
    ...
    public void setItems(List<MovieModel> movieModels) {
...
//    }
    ...
    public void notifySelection(int currentSelection) {
        ...
        movieModels.get(currentSelection).setSelected(true);
    ...
    public void removeLastSelection() {
        if (lastSelection != -1) {
            if (lastSelection > movieModels.size()) {
                lastSelection = -1;
                return;
            }
            if (movieModels.get(lastSelection).isSelected()) {
                movieModels.get(lastSelection).setSelected(false);
